...
- [V] **Pairing connects immediately.** No force-quit. *(`confirm_pairing` left
...
- [V] **It reconnects on its own** after backgrounding the app, after the
...
- [V] **A sleeping computer stops reading as connected** within ~20s.
...
- [V] **The timeline can be dragged** and the track actually moves.
...
- [V] **Errors are a normal iOS alert**, not a floating glass capsule.
- [V] (requires waiting for TCP keepalive) **Bluetooth takes over when Wi-Fi goes off**, within about six seconds
...
- [V] **"Connecting…" while it is connecting.** A dial in flight used to read
      as "Not connected", which beside a state called Connecting says *gave up*.
      "Not connected" now means every route has been tried.
- [V] **A file arriving says it arrived** — "Saved <name>", not "Sent the
...
- [V] **Seeking to 00:00 works.** Fixed on the desktop, so it needs
      `./scripts/install.sh` — a phone build alone will not carry it.
- [V] **Wi-Fi coming back moves the session off Bluetooth**, without a
...
- [V] The app has an icon on the Home Screen, not a white square.
- [V] The icon's corners are rounded by iOS and not black. *(A black corner
...
- [V] Under iOS 26: navigation bars, the tab bar and sheets look like Liquid
      Glass rather than flat iOS 17 chrome.
- [?] (cannot test) Under iOS 17 or 18, if you have such a device: the app still launches and
...
- [V] Three tabs: Devices, Files, Status.
- [V] Scrolling a long list minimises the tab bar (iOS 26 only).
- [V] **The Bluetooth prompt still appears on a fresh install.** Delete the app,
...
- [V] Status shows no device ids or entitlement rows; those are behind Debug.
- [V] Debug still reaches the Bluetooth transcript, and Copy still works.
- [V] **There is no Connect button.** Turn the desktop's Wi-Fi off and watch a
      peer go from Connected → Not connected on its own, and back when it
      returns, with nobody pressing anything.
- [V] A peer mid-handshake shows a spinner and "Connecting…", not "Not
      connected". *Easiest to see by putting the desktop to sleep and waking it.*
- [V] An unreachable peer states a reason under Status — e.g. "Nothing has found
      it yet…". Confirm the reason **disappears** once it connects.
- [?] (replaced with native SwiftUI alert) An error banner appears above the tabs, is dismissible, and clears itself
      after ~30s.
...
- [V] The timeline advances roughly in real time, not in two-second jumps.
- [V] **Drag the timeline.** The track moves to where you dropped it, and the
      knob does not snap back before the new position arrives.
- [?] (cannot test) On a stream with no length, or a player reporting `can_seek = false`, the
      timeline is a plain bar with no draggable knob.
- [V] The volume row shows a percentage that tracks the slider.
- [V] A transport button that the player ignores turns orange rather than
      pretending it worked. *Chromium reporting `CanControl` while ignoring a
      volume write is the known case.*
- [V] Background the app for a minute with something playing, come back, and
      confirm the desktop was not being polled meanwhile *(watch the journal)*.
- [V] Over Bluetooth only, with Wi-Fi off: media still updates, more slowly.

## 4. Lock screen and controls (Stage C)

- [V] Add the circular accessory widget to the Lock Screen; it shows a lock,
      open lock, or computer glyph.
- [V] The rectangular and inline accessory widgets still render.
- [V] Add the **Wake PC** control to Control Centre, and to a Lock Screen
      button slot. Pressing it wakes a sleeping machine **from the Lock Screen,
      without unlocking the phone.**
- [?] (cannot test) With no wake target on file, the control says so rather than doing
      nothing silently.
...
- [V] With two peers, `acryliusctl device ping A` while B is going up and down:
...
- [V] Six digits appear on the phone **and** as a desktop notification, and
      they match.
- [V] The notification carries **They match** and **They don't** as buttons.
- [V] Pressing **They match** on both ends pairs, and a session comes up without
      force-quitting the app.
- [V] Pressing **They don't** on either end pairs nothing, and a second attempt
      from the phone is refused for a few minutes afterwards. *This cooldown is
      load-bearing: see PROTOCOL.md § 8.*
- [?] (cannot test, no third device) While the desktop is showing digits, its row on a **second** phone is
...
- [V] Send a file phone → desktop, and desktop → phone. Accept from the
      notification, and accept from `acryliusctl file accept`.
- [V] Lock and unlock the desktop from the phone; confirm the phone reports the
      truth both times.
- [V] Clipboard both directions.
